...
CK_RV ckteec_invoke_init(void)
{
	...
	e = k_mutex_lock(ta_ctx.init_mutex, K_FOREVER);
	if (e)
...
out:
	e = k_mutex_unlock(ta_ctx.init_mutex);
	if (e) {
		EMSG("k_mutex_unlock: %s", strerror(e));
		EMSG("terminating...");
		exit(EXIT_FAILURE);
	}
...
CK_RV ckteec_invoke_terminate(void)
{
	...
	e = k_mutex_lock(ta_ctx.init_mutex, K_FOREVER);
	if (e) {
		EMSG("k_mutex_lock: %s", strerror(e));
		EMSG("terminating...");
		exit(EXIT_FAILURE);
	}
